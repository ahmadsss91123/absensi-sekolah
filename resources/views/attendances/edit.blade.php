<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-slate-900 dark:text-slate-100">Edit Absensi</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Perbarui data kehadiran siswa dengan mudah.</p>
            </div>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200 dark:bg-slate-950 dark:ring-slate-700">
                <div class="p-6 space-y-6 text-slate-900 dark:text-slate-100">
                    ...
                    <form action="{{ route('attendances.update', $attendance) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                            ...
                            <div>
                                ...
                                <select id="student_id" name="student_id" class="mt-2 block w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" required>
                                    ...
                                    @foreach($students as $student)
                                        <option value="{{ $student->id }}" {{ old('student_id', $attendance->student_id) == $student->id ? 'selected' : '' }}>{{ $student->nama }} ({{ $student->nis }})</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="rounded-3xl bg-slate-50 p-4 text-sm text-slate-600 dark:bg-slate-900 dark:text-slate-300">
                            ...
                            <p class="mt-2">Absensi hanya dapat dicatat satu kali per siswa pada tanggal yang sama.</p>
                        </div>
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:justify-between">
                            ...
                            <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-sky-600 px-4 py-3 text-sm font-semibold text-white hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500">Perbarui Absensi</button>
                        </div>
                    </form>
                </div>
            </div>
